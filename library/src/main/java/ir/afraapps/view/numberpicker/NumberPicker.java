...
public class NumberPicker extends LinearLayout {
  ...
  /**
   * Handle to the reusable command for setting the event_insert text selection.
   */
...
//        }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < selectorIndices.length; i++) {
      ...
      // Do not draw the middle item if event_insert is visible since the event_insert
  ...
  /**
   * The numbers accepted by the event_insert text's {@link Filter}
   */
  ...
  class InputTextFilter extends NumberKeyListener {
    ...
    // XXX This doesn't allow for range limits when controlled by a soft event_insert method!
  ...
  /**
   * Command for setting the event_insert text selection.
   */
